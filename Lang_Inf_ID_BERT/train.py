...
def train_model(model: BertConversationalLanguageClassificationModel, train_dataloader: DataLoader, 
                dev_dataloader: DataLoader, test_dataloader: DataLoader, epochs: int, learning_rate: float) -> float:
    """
...
#Training
    for epoch in tqdm(range(epochs), desc="Epoch Loop"):
        ...
        dev_acc = model_accuracy(model, dev_dataloader, device)
        train_acc  = model_accuracy(model, train_dataloader, device)
        test_acc = model_accuracy(model,test_dataloader, device)
        avg_loss = total_loss/total
        model_accuracy(model, dev_dataloader, device)
        print(f"Epoch {epoch}/{epochs} | Avg loss: {avg_loss:.4f} | Train Accuracy: {train_acc:.4f} | Dev Accuracy: {dev_acc:.4f} | Test Accuracy: {test_acc:.4f}", flush=True)
...
def main():
  ...
  #  parser.add_argument("--filename", default='model_weights.pth', type=str, 
    ...
    train_dataloader = get_dataloader("train_clean", batch_size=args.batch_size, sample=10)
    dev_dataloader = get_dataloader("dev_clean", batch_size=args.batch_size, sample=10);
    test_dataloader = get_dataloader('test_clean',  batch_size=args.batch_size, sample=10)
